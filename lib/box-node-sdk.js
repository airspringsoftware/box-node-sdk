...
function BoxSDKNode(params) {
	...
	if (typeof params.clientID === 'string') {
	        myparams.clientID = params.clientID;
	}

	if (typeof params.clientSecret === 'string') {
	        myparams.clientSecret = params.clientSecret;
	}

	// I'm not sure why we're creating the params object
	// from scratch but for now I'm just adding it here.  We
	// need the appAuth key if we are going to support jwt auth.
	// merge options expects plain object
	if (typeof params.appAuth === 'object') {
		var appAuth = {};
		appAuth.keyID =  params.appAuth.keyID;
		appAuth.privateKey = params.appAuth.privateKey;
		if (typeof  params.appAuth.passphrase === 'string') {
			appAuth.passphrase = params.appAuth.passphrase;
		}
		myparams.appAuth = appAuth;
	}

	if (typeof params.enterpriseID === 'string') {
		myparams.enterpriseID = params.enterpriseID;
	}
	
	this.config = new Config(myparams);
